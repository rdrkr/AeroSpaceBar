...
/// Use case for setting the window corner radius configuration.
///
/// This use case encapsulates the business logic for setting the window corner radius
/// in the configuration gateway. It belongs to the domain layer and follows
/// clean architecture principles.
@MainActor
final class SetWindowCornerRadiusUseCase {
    ...
    /// Executes the use case to set the window corner radius.
    /// - Parameter value: The window corner radius in points
    func execute(_ value: CGFloat) async {
        await configurationGateway.setWindowCornerRadius(value)
    }
}
